...
> **Delivered.** This branch has landed and is delivered as `diagnostic_engine_v9/` in this bundle (engine 0.9.0, 597 tests). The steps below are the integration record - the how and why - not a to-do list. The base it was built from (0.8.0, 588 tests) is provenance, not a build target you diff against.

**For:** implementation record / engineering reviewer
**Built from:** the 0.8.0 base (588 tests); **delivered as:** `diagnostic_engine_v9/` (engine 0.9.0, 597 tests)
...
> **Grounding (historical).** This note and the prebuilt lookup were grounded against the 0.8.0 base tree (588 tests, with `data/` and `inputs/`). That base zip is not shipped in this combined bundle; the delivered code is `diagnostic_engine_v9/`, which already contains the changes merged.
...
1. Rebuild the per-tenant lookup, `inputs/tenant_question_lookup_v2.csv`, so `(tenant, item)` uses seven-field keys: six fields for non-division, seven for division, the seventh being `response_includes_remainder` derived from the stored correct answer (a structured quotient-plus-remainder answer is `True`, a plain quotient is `False`), never from `n1 % n2`. **A finalized seven-field lookup is provided in this bundle at `lookup/tenant_question_lookup_v2.csv` (2,536 rows); drop it into `inputs/`, replacing the six-field baseline.** It was originally re-keyed from the v8 lookup (non-division rows byte-identical, division rows with the flag appended). As of 2026-07-06 it is instead regenerated from the committed builder (`calibration/reference/build_question_lookup.py` + `item_key.py`) against the `20260628` corrected all-tenant bank: 2,536 rows, all joining the seven-field `question_parameters.csv` with zero misses and zero uncalibrated rows, and with seven Private split rows corrected from `_b` to `_z`. The builder is now in the repo under `calibration/reference/`; see `audit/v9_lookup_bank_audit_record.md` for the regeneration record.
   ...
   - **Private x_id, resolved.** The corrected bank explicitly assigns Private the `_z` x_id (identical to Delhi's) for all 7 operands Private serves; the v8 lookup's `_b` was stale. This lookup now maps Private to `_z`. Two secondary observations remain and were deliberately **not** changed, because they are pre-existing v8 serving choices that touch grade-specific and entry-test behavior: (a) Delhi 36/3 uses the grade-3 variant `q_dlg3_div_00611_b`, while the bank also offers `q_div_00611_z` plus grade 4-8 variants; (b) Karnataka 36/3 has an additional entry-test variant `q_entry_div_00611_b` not carried in the lookup. Confirm with the engine owner whether Delhi 36/3 should use `_z` for consistency with the other Delhi operands, and whether the extra variants should be served.
...
The branch landed as one coupled set off the 0.8.0 base and is delivered as `diagnostic_engine_v9/`. Final state and the report-back from the prototyping chat:
...
- `diagnostic_engine_v9/` - the delivered engine (0.9.0, 597 tests), the sole code+data copy. Built from the 0.8.0 base (not shipped here).
