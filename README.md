# v10 Diagnostic Engine Bundle (delivered)

This bundle is the delivered v10 dynamic diagnostic engine plus its specs, calibration and lookup data, and provenance records. The misconception-classifier integration and the mixed-mode (online/offline switching) work have landed: engine **0.10.0, 668 tests green**, with `question_x_id` wire fields and the 667-item calibration. The engine code and its data live in **`diagnostic_engine/`** - the single code+data copy in this bundle.

## Two version numbers (keep them distinct)

- **Engine package version: `0.10.0`** - in `diagnostic_engine/engine/__init__.py`, `pyproject.toml`, and `config/engine_config.yaml`. This is what the engine stamps on every stored session and verdict.
- **Design / document version: `v9`** - the dynamic diagnostic design and its document set (specs, note, audit).

The directory name `diagnostic_engine/` tracks the engine release line (0.10.0). It is only a folder name: the project is `aml-diagnostic-engine` and the importable package is `engine`, so nothing in the code depends on the directory name.

## Provenance (historical, not a build step)

This engine was built from a 0.8.0 base (588 tests) by applying the misconception-integration change set (8 tests), the post-review fixes (P0-1 version stamp, P0-4 `question_x_id` wire rename, P2-4 test-path portability), and the entry-preferring lookup: 588 + 8 + 1 version-stamp = **597**. That base engine (`diagnostic_engine_v8.zip`) is **not shipped** in this combined bundle - the delivered `diagnostic_engine/` already contains it merged. The four classifier module versions (addition 17, subtraction 29, multiplication 20, division 47) are per-module file identifiers and are unchanged.

Start with `v9_engine_integration_note.md` for the implementation record and the as-landed closeout (Sections 5 and 5.1).

## What landed (delivered in `diagnostic_engine/`)
...
3. **Stage B in-process integration.** `engine/stage_b_integration.py` connects mastery verdicts to the misconception classifier (vendored at `diagnostic_engine/stage_b_classifier/`); `tests/test_stage_b_integration.py` covers it, including the `"False"`-string resolver regression and the e2e.
...
| Path | What it is |
|---|---|
| `diagnostic_engine/` | The delivered engine, sole code+data copy: `engine/` (package), `data/question_parameters.csv` (667 items), `inputs/tenant_question_lookup_v2.csv` (2,536 rows), `tests/` (668), `config/`, `stage_b_classifier/` (vendored), `pyproject.toml`. Engine 0.10.0. |
| `v9_engine_integration_note.md` / `.docx` | The implementation record and as-landed closeout. Start here. |
| `applied_change_set/CHANGES.md` | The landed-change manifest (what changed and why), retained as a standalone provenance record. The code it describes is in `diagnostic_engine/`. |
...
| `lookup/tenant_question_lookup_v2.csv` | Provenance copy of the finalized lookup (content-identical to `diagnostic_engine/inputs/tenant_question_lookup_v2.csv`). |
...
|---|---|---|
| `diagnostic_engine/data/question_parameters.csv` | 667 items | Distinct `item` count = 667; zero `\|True` split orphans; byte-identical to `calibration/reference` rev-2 file (md5 `091063c2`). |
| `diagnostic_engine/inputs/tenant_question_lookup_v2.csv` | 2,536 rows | Entry-preferring resolution; content-identical to the `lookup/` provenance copy (0 differing `question_x_id`, 0 flag diffs). |
| Engine version | 0.10.0 | `__init__.py`, `pyproject.toml`, `config/engine_config.yaml` all 0.10.0; verdict-stamp test asserts stored `engine_version == "0.10.0"`. |
| API wire fields | `question_x_id` | `grep question_id engine/api/schemas.py` = 0. |
| Test suite | 668 passing | Re-run in place after assembly; no dead harness. |
